...
API_KEY = "c5b024b83emshfcd51d6e1a79e12p118898jsn1ad10fbc6a2f"
...
# ==========================
# MAIN COLLECTION LOOP (ONLY EXTRA ROWS)
# ==========================

def collect_extra_for_missing_roles(
    extra_to_collect: int = 800,
    pages_per_combo: int = 8,
    pause: float = 1.0,
    save_path: str = SAVE_PATH,
):
    """
    Load existing CSV, then scrape ONLY missing roles and add `extra_to_collect`
    new rows on top (no duplicates).
    """

    # 🔥 Only the roles you said are missing:
    roles = [
        "Data Architect",
        "Analytics Engineer",
        "BI Analyst",
    ...
    # ===== Load existing CSV (MUST exist, you already have ~500 rows) =====
    start_count = 0
    if os.path.exists(save_path):
        ...
        print(f"[WARN] No existing CSV found at {save_path}. This will start from scratch.")

    target_total = start_count + extra_to_collect
    print(f"[INFO] Starting with {start_count} rows. Target total rows: {target_total}")
        ...
        for (loc, ccode) in locations:
            ...
            for page in range(1, pages_per_combo + 1):
                ...
                if len(all_records) >= target_total:
                    print("[INFO] Extra target reached, stopping collection loop.")
                ...
                print(f"[INFO] Collected so far: {new_count_after} rows (+" 
                      f"{new_count_after - new_count_before} this page)")
...
if __name__ == "__main__":
    ...
    df = collect_extra_for_missing_roles(
        extra_to_collect=800,  # how many NEW rows you want to add on top of existing
        pages_per_combo=8,
        pause=1.0,
        save_path=SAVE_PATH,
    )
    ...
    print("\n" + "=" * 60)
    print(f"[INFO] Scraping completed. Total rows now: {len(df)}")
